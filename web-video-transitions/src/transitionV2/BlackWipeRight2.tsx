...
vec4 transitionIn(vec2 uv) {
  vec4 black = vec4(0.0, 0.0, 0.0, 1.0);
  vec2 p=uv.xy/vec2(1.0).xy;
  vec4 a=getFromColor(p);
  return mix(a, black, step(0.0+p.x,u_time));
}

vec4 transitionOut(vec2 uv) {
  vec4 black = vec4(0.0, 0.0, 0.0, 1.0);
  vec2 p=uv.xy/vec2(1.0).xy;
  vec4 b=getToColor(p);
  return mix(b, black, step(1.0-p.x,u_time));
}

void main() {
  if (u_isTransition == false) {
    gl_FragColor = transitionIn(v_texcoord);
  } else {
    gl_FragColor = transitionOut(v_texcoord);
  }
}
`;

const BlackWipeRight2 = ({
  width,
  height,
  startVideoSrc,
  endVideoSrc,
  ...
  const render = (deltaTime: number) => {
    ...
    shader.uniforms.u_isTransition = 0;
    ...
    if (startVideo.duration - startVideo.currentTime < duration / 2) {
      ...
      timeRef.current = linearInterpolation(0.0, 1, t);
      gl.activeTexture(gl.TEXTURE0 + textureUnit2);
      gl.bindTexture(gl.TEXTURE_2D, texture2);
      gl.texImage2D(
        gl.TEXTURE_2D,
    ...
    if (startVideo.duration === startVideo.currentTime) {
      ...
      shader.uniforms.u_isTransition = 1;
      endVideo.play();
      const t = Math.min(
        (deltaTime - timeStampRef.current) / ((duration * 1000) / 2),
        1
      ...
      timeRef.current = linearInterpolation(1, 0, t);
      gl.activeTexture(gl.TEXTURE0 + textureUnit1);
      gl.bindTexture(gl.TEXTURE_2D, texture1);
      gl.texImage2D(
        gl.TEXTURE_2D,
      ...
      timeRef.current = timeRef.current < 0 ? 0 : timeRef.current;
      if (timeRef.current === 0) setIsTransition(false);
      shader.uniforms.u_time = timeRef.current;
    }
...
export default BlackWipeRight2;
